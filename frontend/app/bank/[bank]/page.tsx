...
export default function BankPage() {
  ...
  useEffect(() => {
    const load = async () => {
      const data = await fetchDiscounts({ bank: params.bank });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-ink">Bank Deals: {params.bank}</h1>
        <p className="text-sm text-muted">
          Browse exclusive offers from {params.bank}.
        </p>
      </div>
